...
impl<T: AsBytes> LocatedSpan<T> {
    ...
    /// # extern crate nom;
    /// # use self::nom_locate::LocatedSpan;
    /// # use self::nom::{Slice, FindSubstring};
    /// #
    /// # fn main() {
...
macro_rules! impl_find_token {
    ( $for_type:ty, $fragment_type:ty) => {
        impl<'a, 'b> FindToken<$for_type> for LocatedSpan<$fragment_type> {
            fn find_token(&self, input: $for_type) -> bool {
                self.fragment.find_token(input)
            }
        }
    }
}
...
/// Implement nom::AtEof for a specific fragment type.
///
/// # Parameters
/// * `$fragment_type` - The LocatedSpan's `fragment` type
///
...
/// ````
#[macro_export]
macro_rules! impl_at_eof {
    ( $fragment_type:ty ) => {
        impl<'a> AtEof for LocatedSpan<$fragment_type> {
            fn at_eof(&self) -> bool {
