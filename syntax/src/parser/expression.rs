use super::{ParseContext, SyntaxKind, SyntaxNode, next, push, source};
...
pub(super) fn simple(mut ctx: ParseContext, nodes: &mut Vec<SyntaxNode>) -> u32 {
    ...
    return match next(&mut ctx, nodes) {
...
pub(super) fn identifier(mut ctx: ParseContext, nodes: &mut Vec<SyntaxNode>, head: Token) -> u32 {
    ...
    return match next(&mut ctx, nodes) {
        Some(tok) if tok.kind == Kind::Symbol && source(&ctx, tok) == "(" => function_call(ctx, nodes, head, tok),
        _ => push(&mut ctx, nodes, SyntaxKind::Identifier, head.length)
    }
}

pub(super) fn parenthesized(mut ctx: ParseContext, nodes: &mut Vec<SyntaxNode>, open: Token) -> u32 {
    unimplemented!()
}

pub(super) fn function_call(mut ctx: ParseContext, nodes: &mut Vec<SyntaxNode>, name: Token, open: Token) -> u32 {
